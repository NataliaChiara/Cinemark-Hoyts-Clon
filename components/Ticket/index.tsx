...
const Ticket = ({
  ...
  handleConfirm: (data: SelectSeatsType) => void;
  showSeats: boolean;
}) => {
  ...
  useEffect(() => {
    const movieData = peliculas.find(
      (movie) => movie.slug === SelectedMovie.slug,
    );
    if (movieData) {
      setMovieData({
    ...
    const cantidadDeEntradasMaximoCalc = SelectedMovie.asientos?.filas.reduce(
      (total, fila) =>
        total + fila.asientosTotales - (fila.asientosReservados?.length || 0),
      0,
    );
  ...
  function addEntradas() {
    if (cantidadEntradas.cantidad < cantidadDeEntradasMaximo) {
      setCantidadEntradas({
        cantidad: cantidadEntradas.cantidad + 1,
        valor: (cantidadEntradas.cantidad + 1) * 9000,
      });
    }
  }
      ...
      <div className={s.container}>
        <div className={s.container__pelicula}>
          ...
          <div className={s.container__pelicula__info}>
            <h1 className={s.container__pelicula__info__titulo}>
              {movieData.titulo}
            </h1>
            <span className={s.container__pelicula__info__edad}>
              {movieData.edad}
            </span>
        ...
        <div className={s.container__entrada}>
          <div className={s.container__entrada__info}>
            <span>
              {cantidadEntradas.cantidad} ENTRADA - ${cantidadEntradas.valor}
            </span>
